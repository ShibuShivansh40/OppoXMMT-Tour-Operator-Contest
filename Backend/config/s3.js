...
async function generatePresignedUploadUrl(fileName, fileType, customObjectKey) {
  const bucketName = (process.env.S3_BUCKET_NAME && process.env.S3_BUCKET_NAME.trim()) || 'oppoxmmt-tour-campaign-mumbai';
...
async function getSignedDownloadUrl(mediaUrl) {
  ...
  try {
    ...
    const bucketName = process.env.S3_BUCKET_NAME || 'oppoxmmt-tour-campaign-mumbai';
